  ...
  const docentesFiltrados = docentesCarrera.filter(doc => {
    ...
    return (
      ...
      export const ComunicacionJefatura: React.FC = () => {
        ...
        const careerAssignments = React.useMemo(() => assignments.filter((assignment) => {
          if (!careerId) {
            return false;
          }
        ...
        const teacherCards = React.useMemo(() => {
          ...
          return Array.from(map.values()).map((teacherAssignments) => {
          ...
          }).filter(({ teacher, teacherAssignments, areaNames }) => matches(search, [
        ...
        const studentCards = React.useMemo(() => {
          ...
          return Array.from(map.values()).map((studentAssignments) => ({
        ...
        if (isLoading) {
          return <div className="rounded-lg bg-white p-6 shadow-sm"><p className="text-sm text-gray-500">Cargando comunicación de jefatura...</p></div>;
        }
        ...
        if (!currentDepartmentHead) {
          return <div className="rounded-lg bg-white p-6 shadow-sm"><p className="text-sm text-gray-500">No se encontró el perfil de jefatura asociado a tu usuario.</p></div>;
        }
          ...
          <div className="space-y-6">
            <div className="bg-gradient-to-br from-[#1B5E20] via-[#2E7D32] to-[#43A047] text-white p-6 rounded-xl shadow-md">
              <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
                <div>
                  ...
                  <h2 className="text-2xl font-bold">Comunicación de carrera</h2>
              ...
              <CardContent className="p-4">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-[#F57F17] flex items-center justify-center"><GraduationCap className="w-5 h-5 text-white" /></div>
                    <div>
                  ...
                  <a href={buildMailtoLink('bienestar@ulsa.mx', 'SIBEC - Reporte de Jefatura de Carrera', `Estimada Oficina de Bienestar Estudiantil,\n\nAdjunto el reporte correspondiente a ${careerName}.\n\n\n\n${mailSignature}`)} target="_blank" rel="noopener noreferrer">
                    <Button className="bg-[#1565C0] hover:bg-[#0D47A1] gap-2"><Mail className="w-4 h-4" /> Enviar Reporte <ExternalLink className="w-3 h-3" /></Button>
                  </a>
                </div>
              </CardContent>
            </Card>
            ...
            <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
              <div className="flex gap-2">
                <Button variant={tab === 'docentes' ? 'default' : 'outline'} onClick={() => setTab('docentes')} className={tab === 'docentes' ? 'bg-[#2E7D32] hover:bg-[#1B5E20]' : ''}>
                  <GraduationCap className="w-4 h-4 mr-2" />
                  Docentes ({teacherCards.length})
                </Button>
                <Button variant={tab === 'estudiantes' ? 'default' : 'outline'} onClick={() => setTab('estudiantes')} className={tab === 'estudiantes' ? 'bg-[#2E7D32] hover:bg-[#1B5E20]' : ''}>
              ...
              <div className="relative flex-1 max-w-md">
                ...
                <Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder={tab === 'docentes' ? 'Buscar docente, correo o área...' : 'Buscar estudiante, correo o matrícula...'} className="pl-10" />
              </div>
            </div>
              ...
              <Card className="border-[#2E7D32]/20 bg-gradient-to-r from-[#E8F5E9] to-white">
                <CardContent className="p-4">
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    ...
                    <a href={buildMailtoLink(teacherCards.map(({ teacher }) => teacher.user.email).join(','), 'SIBEC - Comunicado de Jefatura de Carrera', `Estimados docentes,\n\n\n\n${mailSignature}`)} target="_blank" rel="noopener noreferrer">
              ...
              <Card className="border-[#2E7D32]/20 bg-gradient-to-r from-[#E8F5E9] to-white">
                <CardContent className="p-4">
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    ...
                    <a href={buildMailtoLink(studentCards.map(({ student }) => student.user.email).join(','), 'SIBEC - Comunicado de Jefatura de Carrera', `Estimados estudiantes,\n\n\n\n${mailSignature}`)} target="_blank" rel="noopener noreferrer">
                      <Button className="bg-[#2E7D32] hover:bg-[#1B5E20] gap-2"><Mail className="w-4 h-4" /> Enviar a Todos <ExternalLink className="w-3 h-3" /></Button>
                    </a>
                  </div>
                </CardContent>
                  ...
                  <Card key={teacher.id} className="bg-white border-none shadow-sm hover:shadow-lg transition-shadow overflow-hidden">
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-1 flex-wrap">
                            <Badge variant="outline" className="bg-green-100 text-green-800 border-green-300">Docente Responsable</Badge>
                            <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200">{teacherAssignments.length} asignaciones</Badge>
                          </div>
                        </div>
                      </div>
                    </CardHeader>
                    <CardContent className="pt-0 space-y-3">
                      <div className="space-y-1 text-xs text-gray-500">
                        <div className="flex items-center gap-1.5"><Building2 className="w-3.5 h-3.5" /><span>{areaNames.join(' · ')}</span></div>
                      ...
                      <div className="border-t pt-3">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {[
                            ...
                            { label: 'Coordinación', subject: 'SIBEC - Coordinación de Actividades', icon: '🤝' },
                          ].map((quickAction) => (
                  ...
                  <Card key={student.id} className="bg-white border-none shadow-sm hover:shadow-lg transition-shadow overflow-hidden">
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-1 flex-wrap">
                            ...
                            <Badge variant="outline" className="bg-gray-50 text-gray-600 border-gray-300 text-[10px]">{student.student_code}</Badge>
                          </div>
                        </div>
                      </div>
                    </CardHeader>
                    <CardContent className="pt-0 space-y-3">
                      <div className="grid grid-cols-2 gap-2 text-xs">
                        <div className="flex items-center gap-1.5 text-gray-500"><GraduationCap className="w-3.5 h-3.5" /><span className="truncate">{getCareerCode(student.career.name)}</span></div>
                        <div className="flex items-center gap-1.5 text-gray-500"><Building2 className="w-3.5 h-3.5" /><span className="truncate">{assignment?.subarea.area.name ?? 'Sin área'}</span></div>
                      </div>
                      <div className="border-t pt-3">
                        <p className="text-xs text-gray-500 mb-2 font-medium">Enviar correo rápido:</p>
                        <div className="flex flex-wrap gap-1.5">
                          {[
                            ...
                            { label: 'Situación Académica', subject: 'SIBEC - Revisión de Situación Académica', icon: '📊' },
